...
Google Maps embed (`src/components/sections/ContactMap.tsx`, keyless
`google.com/maps?…&output=embed` URL — no API key, no billing account):

- **Consent-aware.** The iframe sets Google cookies, so it is only loaded
  once the visitor has granted *marketing* consent in the cookie banner
  (now, or later — the component listens to `consent-update`) or clicks
  the "Load map" button. Until then the slot is a dark panel with the
  button, a one-line notice and a plain "Open in Google Maps" link
  (`maps/search/?api=1` — no cookies on our page).
- **Location per market.** Swiss locales (de-CH, fr-CH) map QuinLay AG,
  Stierenberg Park 1A, 6221 Rickenbach, and the address box under the map
...
- New messages `contactPage.map.{load,note,open}` in all 16 locale files
  (Swiss files regenerated by the overlays).
- `pageImages.contact` removed — the slot no longer takes a photo.

QuinLay logo: Michael uploaded `public/images/partners/quinlay.png`
(1366×1070, emblem + wordmark, taupe/grey on transparent) straight to
main; the partners card shows it on a 128px white tile (the logo has no
contrast on the black card otherwise) at its own aspect ratio, and the
card renders the tile only when the file exists.

`products/custom-print-edge-to-edge.jpg` was re-uploaded on main as PNG
data under the .jpg name (1492×995, 1.4 MB); re-encoded to a real
progressive JPEG (82 %, same pixels) like the rest of the audit photos.
