...
Re-Sound target per locale (`resoundLocaleFor` in `src/lib/site-config.ts`): be/nl → `/nl`, fr/fr-ch → `/fr`, de/ch → `/de`, es → `/es`, pt → `/pt`, every other locale (en, uk, us, da, sv, no, is, pl) → `/en`. The Nordic Re-Sound locales are `noindex`, so no Nordic domain links into them. Every link into re-sound.be uses the LOCAL route (Michael, 12 Sep 2026): the range page in that Re-Sound locale where its URL is confirmed — Dutch: `/nl/products/pet-akoestische-panelen` (panels), `/nl/products/akoestische-belcabines` (booths), `/nl/products/houten-akoestische-panelen` (wooden panels) — and the locale root otherwise. The fr, de, en, es and pt range URLs are still to be supplied (re-sound.be is unreachable from the build sandbox); they go into `resoundRangePaths` in `src/lib/site-config.ts` and every link follows. The footer company link stays on the locale root.
...
| Locale | URL | H1 | Re-Sound links (section "what the ceiling cannot fix": panels, booths) | Internal links |
|---|---|---|---|---|
| be | https://stretchplafond.be/akoestiek | Hoe verminder ik de nagalm in mijn leslokaal of kantoor? | `PET-wandpanelen van Re-Sound` → https://re-sound.be/nl/products/pet-akoestische-panelen · `akoestische belcabines` → https://re-sound.be/nl/products/akoestische-belcabines | `Acoustic Stretch System` → https://stretchplafond.be/products/acoustic-stretch-system (ceiling section) · CTA to the on-site quote form (`source=acoustics_page`) · breadcrumb Home / Solutions |
| nl | https://stretchplafond.nl/akoestiek | Galm in de kantoortuin: hoeveel haalt een akoestisch plafond eraf? | `de PET-wandpanelen van Re-Sound` → https://re-sound.be/nl/products/pet-akoestische-panelen · `focus- en belcellen` → https://re-sound.be/nl/products/akoestische-belcabines | `Acoustic Stretch System` → https://stretchplafond.nl/products/acoustic-stretch-system (ceiling section) · CTA to the on-site quote form (`source=acoustics_page`) · breadcrumb Home / Solutions |
| fr | https://stretchplafond.fr/acoustique | Comment réduire la réverbération d'une pièce avec un plafond acoustique ? | `panneaux muraux en PET de Re-Sound` → https://re-sound.be/fr · `cabines téléphoniques et de réunion` → https://re-sound.be/fr | `Acoustic Stretch System` → https://stretchplafond.fr/products/acoustic-stretch-system (ceiling section) · CTA to the on-site quote form (`source=acoustics_page`) · breadcrumb Home / Solutions |
| de | https://stretchdecken.de/akustik | Was hilft wirklich gegen Nachhall im Raum? | `PET-Akustikpaneele von Re-Sound` → https://re-sound.be/de · `Telefon- und Meetingboxen` → https://re-sound.be/de | `Acoustic Stretch System` → https://stretchdecken.de/products/acoustic-stretch-system (ceiling section) · CTA to the on-site quote form (`source=acoustics_page`) · breadcrumb Home / Solutions |
...
|---|---|---|---|
| be | `akoestische wandpanelen van Re-Sound` → https://re-sound.be/nl/products/pet-akoestische-panelen <br>from https://stretchplafond.be/products/acoustic-stretch-system | `belcabines voor open kantoren` → https://re-sound.be/nl/products/akoestische-belcabines <br>from https://stretchplafond.be/blog/stretch-ceiling-acoustics-explained | `wandabsorbers en eilanden van Re-Sound` → https://re-sound.be/nl/products/pet-akoestische-panelen · `houten akoestische panelen` → https://re-sound.be/nl/products/houten-akoestische-panelen <br>from https://stretchplafond.be/materials/acoustic-panels |
| nl | `wandpanelen en telefooncellen van Re-Sound` → https://re-sound.be/nl/products/pet-akoestische-panelen <br>from https://stretchplafond.nl/products/acoustic-stretch-system | `concentratiecabines voor de kantoortuin` → https://re-sound.be/nl/products/akoestische-belcabines <br>from https://stretchplafond.nl/blog/stretch-ceiling-acoustics-explained | `akoestische wandabsorbers van Re-Sound` → https://re-sound.be/nl/products/pet-akoestische-panelen · `houten akoestische wandpanelen` → https://re-sound.be/nl/products/houten-akoestische-panelen <br>from https://stretchplafond.nl/materials/acoustic-panels |
...
- Re-Sound range URLs still needed for fr, de, en, es and pt (re-sound.be is unreachable from the build sandbox, so they could not be read off the site): the booths, PET panels and wooden panels pages in each language. Until then those locales link to their Re-Sound locale root. One entry per language in `resoundRangePaths` (`src/lib/site-config.ts`) switches every placement over — nothing else to edit.
