...
import { brand, resoundUrlFor, hasResoundRange } from '@/lib/site-config';
      ...
      <section className="container" style={{ padding: 'clamp(22px,3vw,36px) 0 clamp(28px,3.4vw,44px)' }}>
        ...
        {g.slug === 'acoustic-panels' && (
          ...
          <p style={{ maxWidth: 640, margin: '14px 0 0', fontSize: 15, lineHeight: 1.6, color: 'var(--text-muted)' }}>
            ...
            <a href={resoundUrlFor(locale, 'panels')} style={{ color: 'var(--black)', fontWeight: 600, textDecoration: 'underline', textUnderlineOffset: 3 }}>{t('acousticResoundLink')}</a>
            {t('acousticResoundAfter')}
            {hasResoundRange(locale, 'wood') && (
              <>
                {t('acousticResoundWoodBefore')}
                <a href={resoundUrlFor(locale, 'wood')} style={{ color: 'var(--black)', fontWeight: 600, textDecoration: 'underline', textUnderlineOffset: 3 }}>{t('acousticResoundWoodLink')}</a>
                {t('acousticResoundWoodAfter')}
              </>
            )}
          </p>
        )}
