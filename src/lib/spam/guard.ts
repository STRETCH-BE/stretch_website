// ============================================================================
// LEAD-ROUTE GUARD CHAIN — the one place the public lead endpoints
// (/api/lead, /api/contact, /api/datasheet-request) run their checks, in the
// agreed order: honeypot → rate limit → Turnstile → form token → spam score.
//
// Outcomes:
//   honeypot     → the route responds { ok: true } and stores NOTHING
//                  (today's behaviour — a bot never learns it was caught).
...
//   ok           → proceed; `spam` says whether the lead is flagged
//                  (stored but not delivered) and carries ip/host/ua.
// ============================================================================
...
export async function runLeadGuards(opts: {
  ...
  /** Route prefix for rate-limit keys, e.g. 'lead' | 'contact' | 'datasheet'. */
  routeKey: string;
  ...
  /** 0 exempts this call from the too-fast rule (one-click confirm). */
  ...
  // 1) Honeypot — silent accept, nothing stored (unchanged behaviour).
  ...
  // 3) Turnstile (lead flow: WE verify — auth routes forward to Supabase).
  ...
  if (isTurnstileEnabled()) {
    ...
    const verdict = await verifyTurnstile({ token, host: host ?? '', ip });
